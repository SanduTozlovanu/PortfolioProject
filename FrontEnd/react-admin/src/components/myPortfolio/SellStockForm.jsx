...
function SellStockForm({open, onClose}) {
    ...
    const getOptions = async () => {
        try {
            const response = await axios.get(`${config.url}/portfolio/select`, {
                headers: {
    ...
    const sellStocks = async () => {
        ...
        try {
            await axios.post(`${config.url}/portfolio/stock/sell`,
                {
                    ticker: ticker,
                    price: price,
                    quantity: quantity
    ...
    return (
        <Dialog open={open} onClose={onClose} fullWidth>
            <DialogTitle style={{textAlign: 'center', fontSize: 24, backgroundColor: colors.primary[400]}}>
                Sell Stock
            </DialogTitle>
                    ...
                    <Grid container spacing={3}>
                        <Grid item xs={6} md={6}>
                            <Autocomplete
                                ...
                                renderInput={(params) => <TextField {...params} label="Select Stock to sell"/>}
                            />
                        </Grid>
                        <Grid item xs={6} md={6}>
